...
class HBCReader:
    ...
    scope_desc_data_storage: BytesIO
    textified_data_storage: BytesIO
    string_table_storage: BytesIO

    def get_header_reader(
        self, bytecode_version: int = LATEST_BYTECODE_VERSION
    ) -> type:
    ...
    def read_header_from_buffer(self):
        ...
        if version >= 75:
            ...
            assert (
                sha1(file_data[:-SHA1_NUM_BYTES]).digest()
                == file_data[-SHA1_NUM_BYTES:]
            )
    ...
    def read_bigints(self):
        ...
        bigint_table = (
            self.get_offset_length_pair_reader() * self.header.bigIntCount
        )()
    ...
    def read_regexp(self):
        ...
        self.regexp_table = (
            self.get_offset_length_pair_reader() * self.header.regExpCount
        )()
        ...
        self.regexp_storage = BytesIO(
            self.file_buffer.read(self.header.regExpStorageSize)
        )
    ...
    def read_debug_info(self):
        ...
        self.debug_string_storage = BytesIO(
            self.file_buffer.read(self.debug_info_header.filename_storage_size)
        )
        ...
        if self.header.version >= 98:
            ...
            self.sources_data_storage = BytesIO(
                self.file_buffer.read(sources_data_size)
            )
            self.scope_desc_data_storage = BytesIO(
                self.file_buffer.read(scope_desc_data_size)
            )
        else:
            sources_data_size = self.debug_info_header.scope_desc_data_offset
            scope_desc_data_size = (
                self.debug_info_header.textified_data_offset
                - self.debug_info_header.scope_desc_data_offset
            )
            textified_data_size = (
                self.debug_info_header.string_table_offset
                - self.debug_info_header.textified_data_offset
            )
            string_table_size = (
                self.debug_info_header.debug_data_size
            ...
            self.textified_data_storage = BytesIO(
                self.file_buffer.read(textified_data_size)
            )
            self.string_table_storage = BytesIO(
                self.file_buffer.read(string_table_size)
            )
